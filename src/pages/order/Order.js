...
const Order = observer(() => {
  ...
  const setShops = async () => {
    ...
    const rows = responce.data[0].reduce(
      (rows, key, index) =>
        (index % 3 === 0
          ? rows.push([key])
                  ...
                  {item.map((subItem) => {
                    return (
                      <div
                        ...
                        onClick={() => handleShop(subItem)}
                      >
                        <div className="shop-up">
                          <img src={pharm_logo} alt="" />
                          <p className="shop-title">{subItem.name}</p>
                          <div className="shop-desc">
                            <p className="shopDesc-text">
                              Europharma is a modern convenient drug store where
                              you can buy certified medicines...
                            </p>
